...
// uncomment this to disable SDL sample (might cause compilation issues on some
// systems)
...
static std::array<int, 2>
    g_tile_size{}; // Saved tile size used by the SDL callback sample.
  ...
  public:
    ...
    void on_draw(tcod::Console& console) override {
        ...
        for(int x = 0; x < SAMPLE_SCREEN_WIDTH; ++x) {
            const float x_coef =
                static_cast<float>(x) / (SAMPLE_SCREEN_WIDTH - 1);
            // get the current column top and bottom colors
            const auto top = tcod::ColorRGB{
                TCODColor::lerp(colors_[TOPLEFT], colors_[TOPRIGHT], x_coef)};
            const auto bottom = tcod::ColorRGB{TCODColor::lerp(
                colors_[BOTTOMLEFT], colors_[BOTTOMRIGHT], x_coef)};
            for(int y = 0; y < SAMPLE_SCREEN_HEIGHT; ++y) {
                const float y_coef =
                    static_cast<float>(y) / (SAMPLE_SCREEN_HEIGHT - 1);
                // get the current cell color
                auto curColor =
                    tcod::ColorRGB{TCODColor::lerp(top, bottom, y_coef)};
        ...
        auto textColor =
            tcod::ColorRGB{console.at({SAMPLE_SCREEN_WIDTH / 2, 5}).bg};
        // and invert it
        ...
        // BKGND_MULTIPLY flag
        tcod::print_rect(
            console, {1, 5, SAMPLE_SCREEN_WIDTH - 2, SAMPLE_SCREEN_HEIGHT - 1},
            "The Doryen library uses 24 bits colors, for both background and "
            "foreground.",
            textColor, GREY, TCOD_CENTER, TCOD_BKGND_MULTIPLY);
    }
  ...
  private:
    ...
    std::array<tcod::ColorRGB, 4> colors_{
        {{50, 40, 150},
         {240, 85, 5},
         {50, 35, 240},
         {10, 200, 130}}}; // random corner colors
...
class OffscreenConsole : public Sample {
  public:
    static constexpr auto FRAME_DECOR_SINGLE_PIPE = std::array<int, 9>{
        0x250C, 0x2500, 0x2510, 0x2502, 0x20, 0x2502, 0x2514, 0x2500, 0x2518};
    OffscreenConsole() {
        tcod::draw_frame(
            secondary,
            {0, 0, SAMPLE_SCREEN_WIDTH / 2, SAMPLE_SCREEN_HEIGHT / 2},
            FRAME_DECOR_SINGLE_PIPE, std::nullopt, std::nullopt);
        tcod::print_rect(
            secondary,
    ...
    void on_enter() override {
        ...
        screenshot =
            sampleConsole; // get a "screenshot" of the current sample screen
    }
    void on_event(SDL_Event&) override {}
    void on_draw(tcod::Console& console) override {
        if(SDL_TICKS_PASSED(SDL_GetTicks(),
                            counter + 1000)) { // Once every second.
            counter = SDL_GetTicks();
            x_ += x_dir_;
            y_ += y_dir_;
        ...
        tcod::blit(console, secondary, {x_, y_}, {}, 1.0f,
                   0.75f); // blit the overlapping screen
    }

  private:
    int counter;
    tcod::Console screenshot{SAMPLE_SCREEN_WIDTH, SAMPLE_SCREEN_HEIGHT};
    tcod::Console secondary{SAMPLE_SCREEN_WIDTH / 2,
                            SAMPLE_SCREEN_HEIGHT / 2}; // second screen
...
class LineDrawingSample : public Sample {
  public:
    LineDrawingSample()
        : background_{SAMPLE_SCREEN_WIDTH, SAMPLE_SCREEN_HEIGHT} {
    ...
    void on_draw(tcod::Console& console) override {
        if((blend_flag_ & 0xff) == TCOD_BKGND_ALPH) {
            ...
            const auto alpha =
                float{(1.0f + cosf(SDL_GetTicks() / 1000.0f * 2.0f)) / 2.0f};
        ...
        // render the gradient
        const auto rect_y =
            static_cast<int>((SAMPLE_SCREEN_HEIGHT - 2) *
                             ((1.0f + cosf(SDL_GetTicks() / 1000.0f)) / 2.0f));
        for(int x = 0; x < SAMPLE_SCREEN_WIDTH; ++x) {
            ...
            tcod::draw_rect(console, {x, rect_y, 1, 3}, 0, std::nullopt, col,
                            static_cast<TCOD_bkgnd_flag_t>(blend_flag_));
        }
        // calculate the segment ends
        const float angle = SDL_GetTicks() / 1000.0f * 2.0f;
        const int xo =
            static_cast<int>(SAMPLE_SCREEN_WIDTH / 2 * (1 + cosf(angle)));
        const int yo = static_cast<int>(SAMPLE_SCREEN_HEIGHT / 2 +
                                        sinf(angle) * SAMPLE_SCREEN_WIDTH / 2);
        const int xd =
            static_cast<int>(SAMPLE_SCREEN_WIDTH / 2 * (1 - cosf(angle)));
        const int yd = static_cast<int>(SAMPLE_SCREEN_HEIGHT / 2 -
                                        sinf(angle) * SAMPLE_SCREEN_WIDTH / 2);
        ...
        for(const auto xy : tcod::BresenhamLine({xo, yo}, {xd, yd})) {
            if(console.in_bounds(xy)) {
                TCOD_console_set_char_background(
                    console.get(), xy.at(0), xy.at(1), LIGHT_BLUE,
                    static_cast<TCOD_bkgnd_flag_t>(blend_flag_));
            }
        }
        // print the current flag
        tcod::print(console, {2, 2},
                    tcod::stringf("%s (ENTER to change)",
                                  flag_names_.at(blend_flag_ & 0xff)),
                    WHITE, std::nullopt);
    }

  private:
    tcod::Console background_{
        SAMPLE_SCREEN_WIDTH,
        SAMPLE_SCREEN_HEIGHT};        // The background of this sample.
    ...
    static constexpr auto flag_names_ = std::array<const char*, 13>{
        "TCOD_BKGND_NONE",        "TCOD_BKGND_SET",
  ...
  public:
    ...
    void on_draw(tcod::Console& console) override {
        static const std::vector<std::string> funcName = {
            "1 : perlin noise       ", "2 : simplex noise      ",
            "3 : wavelet noise      ", "4 : perlin fbm         ",
            "5 : perlin turbulence  ", "6 : simplex fbm        ",
            "7 : simplex turbulence ", "8 : wavelet fbm        ",
        ...
        for(int y = 0; y < 2 * SAMPLE_SCREEN_HEIGHT; ++y) {
            for(int x = 0; x < 2 * SAMPLE_SCREEN_WIDTH; ++x) {
                const float f[2] = {zoom * x / (2 * SAMPLE_SCREEN_WIDTH) + dx,
                ...
                switch(func_) {
                    ...
                    case TURBULENCE_PERLIN:
                        value = noise_.getTurbulence(f, octaves_,
                                                     TCOD_NOISE_PERLIN);
                        break;
                    case FBM_SIMPLEX:
                ...
                const auto c =
                    static_cast<uint8_t>((value + 1.0f) / 2.0f * 255);
                // use a bluish color
                img_.putPixel(x, y,
                              tcod::ColorRGB{static_cast<uint8_t>(c / 2),
                                             static_cast<uint8_t>(c / 2), c});
            }
        }
        // blit the noise image on the console with subcell resolution
        tcod::draw_quartergraphics(console, img_);
        // draw a transparent rectangle
        tcod::draw_rect(console, {2, 2, 23, (func_ <= WAVELET ? 10 : 13)}, 0,
                        std::nullopt, GREY, TCOD_BKGND_MULTIPLY);
        ...
        for(int curfunc = PERLIN; curfunc <= TURBULENCE_WAVELET; ++curfunc) {
            ...
            const auto bg =
                curfunc == func_ ? std::optional{LIGHT_BLUE} : std::nullopt;
            tcod::print(sampleConsole, {2, 2 + curfunc}, funcName.at(curfunc),
                        fg, bg);
        }
        // draw parameters
        tcod::print(sampleConsole, {2, 11},
                    tcod::stringf("Y/H : zoom (%2.1f)", zoom), WHITE,
                    std::nullopt);
        if(func_ > WAVELET) {
            tcod::print(sampleConsole, {2, 12},
                        tcod::stringf("E/D : hurst (%2.1f)", hurst_), WHITE,
                        std::nullopt);
            tcod::print(sampleConsole, {2, 13},
                        tcod::stringf("R/F : lacunarity (%2.1f)", lacunarity_),
                        WHITE, std::nullopt);
            tcod::print(sampleConsole, {2, 14},
                        tcod::stringf("T/G : octaves (%2.1f)", octaves_), WHITE,
                        std::nullopt);
        }
    }
...
class FOVSample : public Sample {
  public:
        ...
        switch(event.type) {
            case SDL_KEYDOWN:
                switch(event.key.keysym.sym) {
                    ...
                    case SDLK_KP_MINUS:
                    case SDLK_MINUS:
                        algonum_ = (algonum_ + NB_FOV_ALGORITHMS - 1) %
                                   NB_FOV_ALGORITHMS;
                        break;
                    default:
                        break;
    ...
    void on_draw(tcod::Console& console) override {
        ...
        static constexpr auto SQUARED_TORCH_RADIUS =
        ...
        tcod::print(console, {1, 0},
                    tcod::stringf("IJKL : move around\nT : torch fx %s\nW : "
                                  "light walls %s\n+-: algo %s",
                                  torch_ ? "on " : "off",
                                  light_walls_ ? "on " : "off",
        ...
        map_.computeFov(px_, py_, torch_ ? (int)(TORCH_RADIUS) : 0,
                        light_walls_, (TCOD_fov_algorithm_t)algonum_);
            ...
            for(int x = 0; x < SAMPLE_SCREEN_WIDTH; ++x) {
                ...
                if(!visible) {
                    ...
                    if(!torch_) {
                        ...
                        const float r =
                            (float)((x - px_ + dx) * (x - px_ + dx) +
                                    (y - py_ + dy) * (y - py_ + dy));
                        if(r < SQUARED_TORCH_RADIUS) {
                            // l = 1.0 at player position, 0.0 at a radius of 10
                            // cells
                            const float l =
                                std::clamp((SQUARED_TORCH_RADIUS - r) /
                                                   SQUARED_TORCH_RADIUS +
                                               di,
                                           0.0f, 1.0f);
                            // interpolate the color
                            base =
                                tcod::ColorRGB{TCODColor::lerp(base, light, l)};
                        }
                        light = base;
                    }
...
class ImageSample : public Sample {
  public:
    ...
    void on_draw(tcod::Console& console) override {
        console.clear();
        const float x =
            SAMPLE_SCREEN_WIDTH / 2 + cosf(SDL_GetTicks() / 1000.0f) * 10.0f;
        const float y = (float)(SAMPLE_SCREEN_HEIGHT / 2);
        const float scale_x =
            0.2f + 1.8f * (1.0f + cosf(SDL_GetTicks() / 1000.0f / 2.0f)) / 2.0f;
        ...
        if(elapsed & 1) {
            ...
            // the red channel
            tcod::draw_rect(console, {0, 3, 15, 15}, 0, std::nullopt,
                            {{255, 0, 0}});
            circle_.blitRect(console, 0, 3, -1, -1, TCOD_BKGND_MULTIPLY);
            // the green channel
            tcod::draw_rect(console, {15, 3, 15, 15}, 0, std::nullopt,
                            {{0, 255, 0}});
            circle_.blitRect(console, 15, 3, -1, -1, TCOD_BKGND_MULTIPLY);
            // the blue channel
            tcod::draw_rect(console, {30, 3, 15, 15}, 0, std::nullopt,
                            {{0, 0, 255}});
...
class MouseSample : public Sample {
  public:
    ...
    void on_draw(tcod::Console& console) override {
        auto last_tile =
            last_motion_; // SDL event to be converted into tile coordinates.
        g_context->convert_event_coordinates(last_tile);
        auto sdl_window = g_context->get_sdl_window();

        const auto window_flags =
            sdl_window ? SDL_GetWindowFlags(sdl_window) : 0;
        ...
        tcod::print(
            ...
            tcod::stringf(
                ...
                mouse.xrel, mouse.yrel, mouse_tile.x, mouse_tile.y,
                mouse_tile.xrel, mouse_tile.yrel,
...
class PathfinderSample : public Sample {
  public:
    ...
    void on_event(SDL_Event& event) override {
        ...
        switch(event.type) {
            ...
            case SDL_MOUSEMOTION: {
                ...
                if(0 <= dest_x && dest_x < SAMPLE_SCREEN_WIDTH && 0 <= dest_y &&
                   dest_y < SAMPLE_SCREEN_HEIGHT) {
                    dest_x_ = dest_x;
                    dest_y_ = dest_y;
                }
            } break;
    ...
    void on_draw(tcod::Console& console) override {
        ...
        tcod::print(console, {1, 1},
                    "IJKL / mouse :\nmove destination\nTAB : A*/dijkstra",
                    WHITE, std::nullopt);
        tcod::print(console, {1, 4},
                    using_astar_ ? "Using : A*" : "Using : Dijkstra", WHITE,
                    std::nullopt);
        // draw windows
        for(int y = 0; y < SAMPLE_SCREEN_HEIGHT; ++y) {
            for(int x = 0; x < SAMPLE_SCREEN_WIDTH; ++x) {
        ...
        if(using_astar_) {
            ...
            for(int y = 0; y < SAMPLE_SCREEN_HEIGHT; ++y) {
                for(int x = 0; x < SAMPLE_SCREEN_WIDTH; ++x) {
                    ...
                    if(!wall) {
                        const float d = dijkstra_.getDistance(x, y);
                        sampleConsole.at({x, y}).bg = tcod::ColorRGB{
                            TCODColor::lerp(lightGround, darkGround,
                                            0.9f * d / dijkstra_dist_)};
                    }
                }
            }
...
static bool randomRoom = false; // a room fills a random part of the node or the
                                // maximum available space ?
static bool roomWalls =
    true; // if true, there is always a wall on north & west side of a room
    ...
    bool visitNode(TCODBsp* node, void* userData) {
        ...
        if(node->isLeaf()) {
            ...
            if(maxy == SAMPLE_SCREEN_HEIGHT - 1)
                maxy--;
            if(randomRoom) {
                minx = TCODRandom::getInstance()->getInt(
                    minx, maxx - minRoomSize + 1);
                miny = TCODRandom::getInstance()->getInt(
                    miny, maxy - minRoomSize + 1);
                maxx = TCODRandom::getInstance()->getInt(minx + minRoomSize - 1,
                                                         maxx);
                maxy = TCODRandom::getInstance()->getInt(miny + minRoomSize - 1,
                                                         maxy);
            }
            ...
            // node->x,node->y,node->w,node->h); resize the node to fit its sons
            ...
            if(node->horizontal) {
                ...
                if(left->x + left->w - 1 < right->x ||
                   right->x + right->w - 1 < left->x) {
                    ...
                    int x1 = TCODRandom::getInstance()->getInt(
                        left->x, left->x + left->w - 1);
                    int x2 = TCODRandom::getInstance()->getInt(
                        right->x, right->x + right->w - 1);
                    int y = TCODRandom::getInstance()->getInt(left->y + left->h,
                                                              right->y);
                    vline_up(map, x1, y - 1);
                    hline(map, x1, y, x2);
                    ...
                    int maxx =
                        MIN(left->x + left->w - 1, right->x + right->w - 1);
                ...
                if(left->y + left->h - 1 < right->y ||
                   right->y + right->h - 1 < left->y) {
                    // no overlapping zone. we need a Z shaped corridor
                    int y1 = TCODRandom::getInstance()->getInt(
                        left->y, left->y + left->h - 1);
                    int y2 = TCODRandom::getInstance()->getInt(
                        right->y, right->y + right->h - 1);
                    int x = TCODRandom::getInstance()->getInt(left->x + left->w,
                                                              right->x);
                    ...
                    int maxy =
                        MIN(left->y + left->h - 1, right->y + right->h - 1);
  ...
  public:
    ...
    void on_draw(tcod::Console& console) override {
        ...
        if(generate_ || refresh_) { // dungeon generation
            bsp_.resize(0, 0, SAMPLE_SCREEN_WIDTH,
                        SAMPLE_SCREEN_HEIGHT); // restore the nodes size
            memset(map_, '#',
            ...
            if(generate_) {
                ...
                bsp_.removeSons();
                bsp_.splitRecursive(
                    NULL, bspDepth, minRoomSize + (roomWalls ? 1 : 0),
...
class NameGeneratorSample : public Sample {
  public:
    NameGeneratorSample() {
        ...
        for(const auto& path :
            std::filesystem::directory_iterator("data/namegen")) {
            if(!path.is_regular_file())
                continue;
            if(!(path.path().extension() == ".cfg"))
                continue;
        ...
        switch(event.type) {
            case SDL_KEYDOWN:
                switch(event.key.keysym.sym) {
                    ...
                    case SDLK_MINUS:
                    case SDLK_KP_MINUS:
                        curSet = (curSet + static_cast<int>(sets.size()) - 1) %
    ...
    void on_draw(tcod::Console& console) override {
        ...
        tcod::print(
            console, {1, 1},
            tcod::stringf("%s\n\n+ : next generator\n- : prev generator",
                          sets.at(curSet).c_str()),
            WHITE, LIGHT_BLUE);
        for(int i = 0; i < static_cast<int>(names.size()); ++i) {
            const std::string& name = names.at(i);
            if(name.length() < SAMPLE_SCREEN_WIDTH) {
                tcod::print(console, {SAMPLE_SCREEN_WIDTH - 2, 2 + i}, name,
                            WHITE, std::nullopt, TCOD_RIGHT);
            }
        }
        ...
        if(delay >= 0.5f) {
            ...
            names.emplace_back(
                TCODNamegen::generate(sets.at(curSet).c_str(), true));
        }
    }
...
class SampleRenderer : public ITCODSDLRenderer {
  public:
    void render(void* sdlSurface) {
        ...
        if(delay_ < 0.0f) {
            ...
            if(effectNum_ == 2)
                sdl_callback_enabled =
                    false; // no forced redraw for burn effect
            else
                sdl_callback_enabled = true;
        }
        switch(effectNum_) {
            case 0:
                blur(screen, sample_x, sample_y, SAMPLE_SCREEN_WIDTH * char_w,
                     SAMPLE_SCREEN_HEIGHT * char_h);
                break;
            case 1:
                explode(screen, sample_x, sample_y,
                        SAMPLE_SCREEN_WIDTH * char_w,
                        SAMPLE_SCREEN_HEIGHT * char_h);
                break;
            case 2:
                burn(screen, sample_x, sample_y, SAMPLE_SCREEN_WIDTH * char_w,
                     SAMPLE_SCREEN_HEIGHT * char_h);
                break;
        }
    }
  ...
  protected:
    ...
    void burn(SDL_Surface* screen, int sample_x, int sample_y, int sample_w,
              int sample_h) {
        ...
        for(int x = sample_x; x < sample_x + sample_w; x++) {
            ...
            for(int y = sample_y; y < sample_y + sample_h; y++) {
                ...
                ig += p2[g_idx];
                ib += p2[b_idx];
                p2 += screen->format->BytesPerPixel +
                      screen->pitch; // get pixel at x,y+1
                ir += p2[r_idx];
                ig += p2[g_idx];
                ib += p2[b_idx];
    ...
    void explode(SDL_Surface* screen, int sample_x, int sample_y, int sample_w,
                 int sample_h) {
        int r_idx = screen->format->Rshift / 8;
        int g_idx = screen->format->Gshift / 8;
        int b_idx = screen->format->Bshift / 8;
    ...
    void blur(SDL_Surface* screen, int sample_x, int sample_y, int sample_w,
              int sample_h) {
            ...
            for(int y = sample_y; y < sample_y + sample_h; y++) {
                ...
                switch(dec) {
                    case 4:
                        ...
                        p -=
                            2 *
                            screen->format->BytesPerPixel; // get pixel at x+2,y
                        ir += p[r_idx];
                        ig += p[g_idx];
                        ib += p[b_idx];
                        ...
                        p +=
                            2 *
                            screen->format->BytesPerPixel; // get pixel at x,y+2
                        ir += p[r_idx];
                        ig += p[g_idx];
                        ib += p[b_idx];
                        p += 2 * screen->pitch;
                    ...
                    case 2:
                        ...
                        ib += p[b_idx];
                        p -=
                            screen->format->BytesPerPixel; // get pixel at x-1,y
                        ir += p[r_idx];
                        ig += p[g_idx];
                        ib += p[b_idx];
                        ...
                        ib += p[b_idx];
                        p +=
                            screen->format->BytesPerPixel; // get pixel at x,y-1
                        ir += p[r_idx];
                        ig += p[g_idx];
                        ib += p[b_idx];
                        p += screen->pitch;
...
class SDLSample : public Sample {
  public:
    void on_enter() override {
        ...
        tcod::print_rect(
            sampleConsole, {0, 3, SAMPLE_SCREEN_WIDTH, 0},
            "The SDL callback gives you access to the screen surface so that "
            "you can alter the pixels one by one using SDL "
            "API or any API on top of SDL. SDL is used here to blur the sample "
            "console.\n\nHit TAB to enable/disable the "
            "callback. While enabled, it will be active on other samples "
                ...
                switch(event.key.keysym.sym) {
                    ...
                    case SDLK_KP_TAB:
                        ...
                        if(sdl_callback_enabled) {
                            TCODSystem::registerSDLRenderer(
                                new SampleRenderer());
                        } else {
...
int main(int argc, char* argv[]) {
    ...
    auto root_console =
        tcod::Console{80, 50}; // The main console to be presented.
    ...
    while(true) {
        ...
        if(!creditsEnd) {
            creditsEnd = TCOD_console_credits_render_ex(root_console.get(), 56,
                                                        43, false, delta_time);
        }
        ...
        for(int i = 0; i < static_cast<int>(samples.size()); ++i) {
            ...
            tcod::print(root_console,
                        {2, 46 - (static_cast<int>(samples.size()) - i)},
                        samples.at(i).name, fg, bg);
        }
        // print the help message
        tcod::print(root_console, {79, 46},
                    tcod::stringf("last frame : %3.0f ms (%3.0f fps)",
                                  delta_time * 1000.0f, timer.get_mean_fps()),
                    GREY, std::nullopt, TCOD_RIGHT);
        tcod::print(root_console, {79, 47},
                    tcod::stringf("elapsed : %8dms %4.2fs", SDL_GetTicks(),
                                  SDL_GetTicks() / 1000.0f),
                    GREY, std::nullopt, TCOD_RIGHT);
        tcod::print(root_console, {2, 47}, "↑↓ : select a sample", GREY,
                    std::nullopt);
        {
            auto sdl_window = g_context->get_sdl_window();
            if(sdl_window) {
                ...
                tcod::print(root_console, {2, 48},
                            tcod::stringf("ALT-ENTER : switch to %s",
                                          is_fullscreen ? "windowed mode  "
                                                        : "fullscreen mode"),
                            GREY, std::nullopt);
            }
        }
        ...
        tcod::blit(root_console, sampleConsole,
                   {SAMPLE_SCREEN_X, SAMPLE_SCREEN_Y});
        /* display renderer list and current renderer */
        tcod::print(root_console,
                    {42, 46 - (static_cast<int>(RENDERERS.size()) + 1)},
                    "Renderer :", GREY, BLACK);

        for(int i = 0; i < static_cast<int>(RENDERERS.size()); ++i) {
            const bool is_current_renderer =
                RENDERERS.at(i).renderer == g_context->get_renderer_type();
        ...
        while(SDL_PollEvent(&event)) {
            switch(event.type) {
                ...
                case SDL_KEYDOWN:
                        ...
                        default:
                            if(SDLK_F1 <= event.key.keysym.sym &&
                               event.key.keysym.sym <
                                   SDLK_F1 + RENDERERS.size()) {
                                auto sdl_window = g_context->get_sdl_window();
                                if(sdl_window) {
                                    // Save fullscreen and maximized state to
                                    // params, so that they are kept on the new
                                    // renderer.
                                    const auto current_flags =
                                        SDL_GetWindowFlags(sdl_window);
                                    static constexpr auto TRACKED_FLAGS =
                                        SDL_WINDOW_FULLSCREEN_DESKTOP |
                                        SDL_WINDOW_MAXIMIZED;
                                    params.sdl_window_flags =
                                        (params.sdl_window_flags &
                                         ~TRACKED_FLAGS) |
                                        (current_flags & TRACKED_FLAGS);
                                }
                                params.renderer_type =
                                    RENDERERS.at(event.key.keysym.sym - SDLK_F1)
                                        .renderer;
                                g_context = nullptr;
                                g_context = tcod::new_context(params);
                            }
                    ...
                    break;
                case SDL_DROPFILE: { // Change to a new tileset when one is
                                     // dropped on the window.
                    tcod::Tileset new_tileset;
                    if(str_ends_with(event.drop.file, ".bdf")) {
                        new_tileset =
                            tcod::Tileset(tcod::load_bdf(event.drop.file));
                    } else if(str_ends_with(event.drop.file, "_tc.png")) {
                        new_tileset = tcod::load_tilesheet(
                            event.drop.file, {32, 8}, tcod::CHARMAP_TCOD);
                    } else {
                        new_tileset = tcod::load_tilesheet(
                            event.drop.file, {16, 16}, tcod::CHARMAP_CP437);
                    }
                    if(new_tileset.get()) {
                        tileset = std::move(new_tileset);
                        params.tileset = tileset.get();
                        TCOD_context_change_tileset(g_context.get(),
                                                    tileset.get());
                    }
